...
#         pass
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        flattened_mask = np.ones(len(flattened_weights))
        flattened_mask[flattened_weights < thresh] = 0.0
        self.mask = flattened_mask.reshape(np_weight.shape)
        ...
        self.linear.weight.data = torch.Tensor(np_weight).to(device)
        # Compute sparsity (Your code: 1 Line)
        self.sparsity = len(np.where(flattened_mask==0)[0]) / len(flattened_weights)
    ...
    def prune_by_std(self, s=0.25):
        ...
        # Generate a mask to determine which weights should be pruned (Your code: <=3 Lines)
        flattened_mask = np.ones(len(flattened_weights))
        flattened_mask[np.abs(flattened_weights) < thresh] = 0.0
        self.mask = flattened_mask.reshape(np_weight.shape)
        # Multiply weight by mask (Your code: 1 Line) 
        np_weight = np_weight * self.mask
        # Copy back to linear.weight and assign to device (Your code: 1 Line)
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        np_weight = np_weight * self.mask
        # Copy back to linear.weight and assign to device (Your code: 1 Line)
        self.conv.weight.data = torch.Tensor(np_weight).to(device)
        # Compute sparsity (Your code: 1 Line)
        self.sparsity = len(np.where(flattened_mask==0)[0]) / len(flattened_weights)
        # Copy mask to device for faster computation (Your code: 1 Line)
        self.mask = torch.Tensor(self.mask).to(device)
